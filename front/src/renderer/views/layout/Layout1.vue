...
<div>
  <!-- <div class="app-wrapper" :class="classObj"> -->
    ...
    <video id="video" v-show="false"></video>
</div>
  <!-- </div> -->
</template>
...
export default {
  ...
  mixins: [ResizeMixin],
  methods: {
    initStudent() { // 用于启动学生webrtc
      console.log('typeof stuRTC ' + this.stuRTC)
      if (this.stuRTC === null) {
        CookieUtils.getCookie(ip.address())
        if (this.roles[0] === 'student' || CookieUtils.cookie.value === 'student') {
          this.initRTC()
    ...
    bindSocketMessage() {
      ...
      this.socket.addMessageType('monitorOffer', body => {
        ...
        if (typeof (this.stuRTC) !== 'undefined' &&
             this.stuRTC !== null) {
      ...
      this.socket.addMessageType('stepInfo', (body) => {
        ...
        const data = body.data
        console.log('step: ' + data.currentStep + '  fromIp: ' + fromIp + ' grade_sum:' + data.grade_sum)
    ...
    startSnapshot(interval) {
      ...
      if (that.roles[0] === 'undefined' ? (CookieUtils.getCookie(ip.address()) === 'student') : true) {
        ...
        setInterval(() => {
          ...
          // 定时采集截图
            that.canvas.getContext('2d').drawImage(that.video, 0, 0, canvas.width, canvas.height)
            const src = that.canvas.toDataURL('image/png', 0.7)
            console.log('发送一个图片帧')
            // console.log(src)
            that.socket.sendMessageToOne({ image: src }, 'snapShot', that.teachers[0].ip, 'teacher')
          } else {
            console.log('教师未登陆')
          }
        }, interval * 1000)
